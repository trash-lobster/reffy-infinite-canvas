...
describe('readJSONFile', () => {
    it('should parse JSON from file', async () => {
        ...
        const file = { text: vi.fn().mockResolvedValue('{"foo":123}') } as unknown as File;
...
describe('convertToPNG', () => {
    it('should resolve with PNG data URL', async () => {
        ...
        vi.stubGlobal('Image', class {
            constructor() {
                setTimeout(() => { if (this.onload) this.onload(); }, 0); return this;
            }
...
describe('mergeMultiImg', () => {
    it('should merge images and return PNG data URL', async () => {
        ...
        const canvas = { width: 0, height: 0, getContext: vi.fn(() => ctx), toDataURL: vi.fn(() => 'data:image/png;base64,merged') };
...
describe('mergeImagesToCanvas', () => {
    ...
    it('should throw if no images', async () => {
        const { mergeImagesToCanvas } = await import('../../../src/util/files');
        await expect(mergeImagesToCanvas([])).rejects.toThrow('No images to merge.');
    });
});
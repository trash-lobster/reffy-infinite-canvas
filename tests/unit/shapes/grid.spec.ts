...
describe("Grid basics", () => {
  it("getPositions returns fullscreen big triangle coords", () => {
    ...
    expect(Array.from(pos)).toEqual([-1.0, -1.0, 3.0, -1.0, -1.0, 3.0]);
  });
...
describe("Grid render", () => {
  ...
  it("throws if buffer is not defined", () => {
    ...
    expect(() => grid.render(gl, program)).toThrowError(
      "Failed to create grid buffer",
    );
  });
  ...
  it("throws if a_Position not found", () => {
    ...
    expect(() => grid.render(gl, program)).toThrowError(
      "Attribute a_Position not found in grid program",
    );
  });
});

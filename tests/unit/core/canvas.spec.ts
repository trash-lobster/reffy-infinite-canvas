...
vi.mock('../../../src/serializer', () => ({
	...
	deserializeCanvas: vi.fn(async (_data, _canvas, _getFile) => ({ mock: 'deserialized' })),
}));
...
describe('Canvas', () => {
	...
	it('render clears, sets grid uniform and renders children with program switching', () => {
		...
		// Grid uniform set
		expect(gl.getUniformLocation).toHaveBeenCalledWith(expect.anything(), 'u_z');
	...
	it('addImageToCanvas emits save and change, returns Img', async () => {
		...
		const img = await canvas.addImageToCanvas('data:image/png;base64,i', 10, 20);
    ...
    it('addImageToCanvas and pass in center to explore onload', async () => {
		...
		const img = await canvas.addImageToCanvas('data:image/png;base64,i', 10, 20, 1, 1, true);
		expect(img).toBeInstanceOf(Img);
        expect(convertToPNG).toHaveBeenCalled();
		expect(saveSpy).toHaveBeenCalled();
		expect(emitSpy).toHaveBeenCalledWith('save');
...
describe('Canvas getters', () => {
    ...
    it('get selected', () => {
        ...
        expect(canvas.getSelected()).toStrictEqual(canvas.selectionManager.selected);
    });
...
describe('setShapeZOrder', () => {
    it('ends function early when there is no selected box', () => {
        ...
        expect((canvas.children as Shape[]).map(c => c.renderOrder)).toStrictEqual(orders);
    });
...
describe('WebGL stats', () => {
	it('wrapWebGLContext increments texture and shader counters', () => {
		...
		expect(stats.texturesLeaked).toBe(stats.texturesCreated - stats.texturesDeleted);
		expect(stats.shadersLeaked).toBe(stats.shadersCreated - stats.shadersDeleted);
	});
	...
	it('getWebGLStats reports program and buffer leak deltas', () => {
		...
		expect(stats.programsLeaked).toBe(stats.programsCreated - stats.programsDeleted);
		expect(stats.buffersLeaked).toBe(stats.buffersCreated - stats.buffersDeleted);
	});
});

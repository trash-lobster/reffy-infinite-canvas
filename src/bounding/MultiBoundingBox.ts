...
const HANDLE_TYPES: BoundingBoxCollisionType[] = [
  ...corners,
  ...sides,
] as BoundingBoxCollisionType[];
...
export class MultiBoundingBox {
  ...
  resize(
    ...
    const factor = direction.includes("LEFT")
      ? 1 - changeInXScale
    ...
    if (
      willFlip(this.scale[0], factor, min) ||
      willFlip(this.scale[1], factor, min)
    )
      return;
    ...
    for (const target of this.targets) {
      ...
      const sX =
        Math.abs(worldScaleX) < min
          ? worldScaleX < 0
            ? -min
            : min
          : worldScaleX;
      const sY =
        Math.abs(worldScaleY) < min
          ? worldScaleY < 0
            ? -min
            : min
          : worldScaleY;
  ...
  normalize(type: NormalizeOption, mode: NormalizeMode) {
    ...
    const goal =
      type === "height"
        ? mode === "first"
          ...
          : this.targets.reduce((a, b) => a + Math.abs(b.height * b.sy), 0) /
            this.targets.length
        : type === "width"
          ? mode === "first"
            ? reference.width * reference.sx
            : this.targets.reduce((a, b) => a + Math.abs(b.width * b.sx), 0) /
              this.targets.length
          : type === "scale"
            ? mode === "first"
              ? reference.sx
              : this.targets.reduce((a, b) => a + Math.abs(b.sx), 0) /
                this.targets.length
            : mode === "first"
              ? reference.width * reference.height * reference.sx * reference.sy
              : this.targets.reduce(
                  (a, b) => a + Math.abs(b.sx * b.width * b.height * b.sy),
                  0,
                ) / this.targets.length; // size calculation aims to get the same area for both
    ...
    for (const target of this.targets) {
      ...
      if (type === "height") {
        ...
        // get current area
        const currentArea =
          target.width * target.height * target.sx * target.sy;
  ...
  getPositions(): number[] {
    return [
      ...
      this.y, // Top-right
      this.x + this.width,
      this.y + this.height, // Bottom-right
      this.x,
      this.y + this.height, // Bottom-left
    ];
  }
  ...
  private getBounds() {
    ...
    for (const rect of arr) {
      ...
      const [endX, endY] = applyMatrixToPoint(
        rect.worldMatrix,
        rect.width,
        rect.height,
      );

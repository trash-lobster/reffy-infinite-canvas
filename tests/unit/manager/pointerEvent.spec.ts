...
if (typeof (globalThis as any).ClipboardEvent === 'undefined') {
	...
	(globalThis as any).ClipboardEvent = ClipboardEventPolyfill as unknown as typeof ClipboardEvent;
}
...
function makeState() {
  return {
    ...
    toggleMode: vi.fn(function (this: any) {
      this.mode = this.mode === PointerMode.SELECT ? PointerMode.PAN : PointerMode.SELECT;
    }),
...
describe('PointerEventManager', () => {
	...
	let assignEventListener: (type: string, fn: any, options?: boolean | AddEventListenerOptions) => void;
	...
	it('registers pointer listeners and updates cursor on move', () => {
		...
		expect(assignEventListener).toHaveBeenCalledWith('pointermove', expect.any(Function));
	...
	it('selection drag creates a multi transform command on pointerup', () => {
		...
		selectedRect.x = 10; selectedRect.y = 20; selectedRect.sx = 2; selectedRect.sy = 3;
	...
	it('onPointerMoveWhileDown computes deltas, calls selectionPointerMove, updates last world coord and cursor', () => {
		...
		const move = new PointerEvent('pointermove', { clientX: 14, clientY: 15, buttons: 1 });
		document.dispatchEvent(move);

		// wx,wy = [15,17]; dx=15-11=4, dy=17-12=5
		expect(selMove).toHaveBeenCalledWith(11 - 15, 12 - 17, 4, 5, st.resizingDirection);
		...
		selMove.mockClear();
		const moveRight = new PointerEvent('pointermove', { clientX: 20, clientY: 25, buttons: 2 });
	...
	it('constructor registers copy handler that calls util.copy when menu inactive', async () => {
		...
		const addSpy = vi.spyOn(window, 'addEventListener').mockImplementation(((type: string, listener: EventListener, options?: boolean | AddEventListenerOptions) => {
			if (type === 'copy') {

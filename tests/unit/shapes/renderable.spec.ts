...
describe("WebGLRenderable internals", () => {
  ...
  it("setUpUniforms stores uniform locations", () => {
    ...
    const gl = {} as WebGLRenderingContext;
    const program = {} as WebGLProgram;
    gl.getUniformLocation = vi
      .fn()
      .mockImplementation((_p: any, name: string) => ({ name }) as any);
  ...
  it("updateUniforms writes resolution and matrix uniforms", () => {
    ...
    (rect as unknown as RectTestHooks).resolutionLocation =
      {} as WebGLUniformLocation;
    (rect as unknown as RectTestHooks).matrixLocation =
      {} as WebGLUniformLocation;
    rect.setWorldMatrix([1, 0, 0, 1, 0, 0]);

    (rect as unknown as RectTestHooks).updateUniforms(gl);

    expect((gl as any).uniform2f).toHaveBeenCalledWith(
      (rect as any).resolutionLocation,
      640,
      480,
    );
    expect((gl as any).uniformMatrix3fv).toHaveBeenCalledWith(
      (rect as any).matrixLocation,
      false,
      rect.worldMatrix,
    );
  });

...
const bounding = vi.hoisted(() => {
  ...
  class MockMarqueeSelectionBox {
    ...
    getBoundingBox = vi
      .fn()
      .mockReturnValue({ left: 0, top: 0, right: 10, bottom: 10 });
  }
  return {
    AABB_isColliding,
    MockBoundingBox,
    MockMultiBoundingBox,
    MockMarqueeSelectionBox,
  };
});
...
const sceneCmd = vi.hoisted(() => ({
  makeMultiRemoveChildCommand: vi.fn(() => ({
    label: "Remove",
    do: vi.fn(),
    undo: vi.fn(),
  })),
...
const flipCmd = vi.hoisted(() => ({
  makeMultiFlipCommand: vi.fn((transforms: any, dir: any, mbb?: any) => ({
    label: "Flip",
    transforms,
    dir,
    mbb,
...
const transformCmd = vi.hoisted(() => ({
  makeMultiTransformCommand: vi.fn((transforms: any) => ({
    label: "Transform",
    transforms,
  })),
}));
...
describe("SelectionManager", () => {
  ...
  it("add() builds boxes, sets passive, and creates multi", () => {
    ...
    (bounding.MockBoundingBox as any).instances.forEach((bb: any) =>
      expect(bb.setPassive).toHaveBeenCalled(),
    );
  ...
  it("remove() updates sets, multi clears when <=1 box", () => {
    ...
    expect(
      instances[0].setActive.mock.calls.length +
        instances[1].setActive.mock.calls.length,
    ).toBeGreaterThan(0);
    expect(m.multiBoundingBox).toBeFalsy();
  });
  ...
  it("deleteSelected() destroys and pushes multi remove", () => {
    ...
    expect(sceneCmd.makeMultiRemoveChildCommand).toHaveBeenCalledWith(canvas, [
      r1,
      r2,
    ]);
  ...
  it("hitTest prefers multi then single, else null", () => {
    ...
    (bounding.MockBoundingBox as any).instances[0].hitTest.mockReturnValue(
      "TOP_LEFT",
    );
  ...
  it("update() and render() call box methods and GL", () => {
    ...
    m.update();
    (bounding.MockBoundingBox as any).instances.forEach((bb: any) =>
      expect(bb.update).toHaveBeenCalled(),
    );
    m.render();
    ...
    (bounding.MockBoundingBox as any).instances.forEach((bb: any) =>
      expect(bb.render).toHaveBeenCalled(),
    );
  ...
  it("move() emits Change and calls move", () => {
    ...
    m.add([r1]);
    m.move(5, 6);
    expect(bounding.MockBoundingBox.instances[0].move).toHaveBeenCalledWith(
      5,
      6,
    );
  ...
  it("resize() branches and emits Change", () => {
    ...
    expect(bounding.MockBoundingBox.instances[0].resize).toHaveBeenCalledWith(
      3,
      4,
      "TOPLEFT",
    );
  ...
  it("flip() pushes Flip with correct transforms", () => {
    ...
    expect(flipCmd.makeMultiFlipCommand).toHaveBeenCalledWith(
      [{ tx: 1 }],
      "horizontal",
    );
    expect(history.push).toHaveBeenCalledWith(
      expect.objectContaining({ label: "Flip" }),
    );
    const r2: any = {
      id: "r2",
      sx: 1,
  ...
  it("alignSelection() and normalize() push Transform", () => {
    ...
    expect(transformCmd.makeMultiTransformCommand).toHaveBeenCalledWith([
      { ax: 1 },
    ]);
  ...
  it("onPointerMove branches: pan, resize, marquee, move", () => {
    ...
    const c1: any = {
      id: "c1",
      getBoundingBox: vi.fn().mockReturnValue({ id: "b1" }),
    };
    const c2: any = {
      id: "c2",
      getBoundingBox: vi.fn().mockReturnValue({ id: "b2" }),
    };
    getCanvasChildren = () => [c1, c2];
    const m2 = new SelectionManager(
      history,
      eventHub,
      gl,
      program,
  ...
  it("tests hitTestAdjustedCorner normally with a single boundingBox", () => {
    ...
    m.boundingBoxes.forEach(
      (box) => (box.hitTest = vi.fn().mockReturnValue("TOPLEFT")),
    );

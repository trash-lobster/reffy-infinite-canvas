...
describe('context menu manager', () => {
    ...
    let isMultiBoundingBoxHit: (x: number, y: number) => BoundingBoxCollisionType | '';
    ...
    it('constructs context menu manager and called methods', () => {
        ...
        expect(assignEventListener).toHaveBeenCalledWith('contextmenu', manager.customContextMenu);
    });
    ...
    it('calls the customContextMenu and emits open multi menu', () => {
        ...
        expect(eventHub.emit).toHaveBeenCalledWith(ContextMenuEvent.Open, 100, 500, 'multi');
    });
    ...
    it('calls the customContextMenu and emits open canvas', () => {
        ...
        expect(event.preventDefault).toHaveBeenCalledOnce();
        expect(event.stopPropagation).toHaveBeenCalledOnce();
        expect(eventHub.emit).toHaveBeenCalledWith(ContextMenuEvent.Open, 100, 500, 'canvas');
    });

...
describe('clipboard', () => {
    ...
    it('copy should fallback to execCommand if clipboard.write fails', async () => {
        (globalThis.navigator.clipboard.write as any).mockRejectedValueOnce(new Error('fail'));
        const img = new Img({ src: 'foo', x: 1, y: 2, sx: 1, sy: 1 });
        await clipboard.copy([img]);
    ...
    it('paste should call getWorldCoords if isWorldCoord is false', async () => {
        const worldCoordsSpy = vi.spyOn(Camera, 'getWorldCoords').mockImplementation((x, y, canvas) => [100, 100]);
    ...
    it('paste finds text/html and does nothing', async () => {
        ...
        const canvas = {
            canvas: {},
            gl: null,
            addImageToCanvas: vi.fn().mockResolvedValue((base64 : string, w: number, y : number) => {}),
        };
    ...
    it('paste finds image/svg and creats html element', async () => {
        const pngConversionSpy = vi.spyOn(Util, 'convertToPNG').mockResolvedValue('mockpngstring');

...
export class InfiniteCanvasElement extends LitElement {
  ...
  assignCanvasStorage(
    ...
    saveFrequency: number = this.#saveFrequency,
  ) {
  ...
  async addImages(fileList: FileList) {
    ...
    const newImages = await innerAddImages(fileList, (src: string) =>
      this.#canvas.addImageToCanvas(src, wx, wy, 1, 1, true),
    );
  ...
  private handleGlobalPointerDown = (e: PointerEvent) => {
    if (
      ...
      !this.renderRoot.contains(e.target as Node)
    ) {

...
import { serializeNode, serializeCanvas, deserializeCanvas, SerializedCanvas, SerializedImg, SerializedRect, SerializedGrid, SerializedNode, SerializedCamera } from '../../../src/serializer/serializer';
...
function makeCanvasStub() {
	...
	const canvas = {
		...
		appendChild: vi.fn(function (this: Canvas, child: Renderable) { this.children.push(child); }),
	} as any;
...
if (!canvasProto.getContext || !canvasProto.getContext.__isMock) {
  canvasProto.getContext = vi.fn(function (this: HTMLCanvasElement, type: string) {
    if (type === '2d') {
...
describe('serializer serializeNode', () => {
	...
	it('serializes Img with id, renderOrder, size, fileId', () => {
		const img = new Img({ x: 0, y: 0, width: 64, height: 64, src: 'data:image/png;base64,iVBORw0...' });
		img.renderOrder = 5;
		img.fileId = 1234;
...
describe('serializer deserializeCanvas', () => {
	...
	it('rebuilds Rect nodes with transform and appends to canvas', async () => {
		...
		const getFile = vi.fn(async (id: number | string) => ({ id, dataURL: 'data:image/png;base64,iVBORw0...' } as any));
	...
	it('rebuilds Img nodes, preserves renderOrder, calls writeFileToDatabase and loads actual file', async () => {
		...
		const getFile = vi.fn(async (id: number | string) => ({ 
            id, 
            dataURL: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAIAQMAAAD+wSzIAAAABlBMVEX///+/v7+jQ3Y5AAAADklEQVQI12P4AIX8EAgALgAD/aNpbtEAAAAASUVORK5CYII=' 
        } as any));
	...
	it('continues when image file lookup fails (uses placeholder path)', async () => {
		...
		const getFile = vi.fn(async (fileId: string | number) => { throw new Error('not found'); });
		await deserializeCanvas(data, canvas, getFile);

...
import {
  ...
  ContextMenuGroup,
  ContextMenuElement,
} from "../../../src/contextMenu/ContextMenu";
...
describe("ContextMenu", () => {
  ...
  it("prevents native contextmenu on menu root", () => {
    const props = {
      options: [{ childOptions: [{ text: "A", onClick: vi.fn() }] }],
    };
    const menu = new ContextMenu(props, root);
...
describe("ContextMenuGroup", () => {
  ...
  it("creates child option elements and appends to group", () => {
    const groupProps = {
      childOptions: [
        { text: "X", onClick: vi.fn() },
        { text: "Y", onClick: vi.fn() },
      ],
    };
...
describe("ContextMenuElement", () => {
  ...
  it("adds click handler when provided", () => {
    ...
    const onClick = vi.fn();
    const el = new ContextMenuElement(
      { text: "Click Me", onClick, parent: group },
      root,
    );
    el.el.click();
  ...
  it("shows a sub-menu on hover and positions within host", () => {
    const group = new ContextMenuGroup({ childOptions: [] }, root);
    const subProps = {
      options: [{ childOptions: [{ text: "Sub1", onClick: vi.fn() }] }],
    };
    const el = new ContextMenuElement(
      { text: "Parent", parent: group, subMenu: subProps },
      root,
    );
...
describe("onpointerenter", () => {
  ...
  it("creates a submenu on hover and positions within host bounds", () => {
    ...
    vi.spyOn(el.el, "getBoundingClientRect").mockReturnValue({
      left: 150,
      top: 150,
      right: 165,
      bottom: 165,
  ...
  it("replaces existing submenu when hovering a different item", () => {
    const group = new ContextMenuGroup({ childOptions: [] }, root);
    const first = new ContextMenuElement(
      {
        text: "First",
        parent: group,
        subMenu: {
    ...
    const second = new ContextMenuElement(
      {
        text: "Second",
        parent: group,
        subMenu: {
          options: [{ childOptions: [{ text: "B", onClick: vi.fn() }] }],
        },
      },
      root,
    );
    ...
    vi.spyOn(root, "getBoundingClientRect").mockReturnValue(hostRect);
    vi.spyOn(first.el, "getBoundingClientRect").mockReturnValue({
      left: 10,
      top: 10,
      right: 30,
      bottom: 30,
    ...
    vi.spyOn(second.el, "getBoundingClientRect").mockReturnValue({
      left: 40,
      top: 40,
      right: 60,
      bottom: 60,

...
describe("Img", () => {
  ...
  it("initialises texture data and uniforms when image is ready", () => {
    ...
    (img as unknown as ImageTestHook)._image = {
      complete: true,
      naturalWidth: 8,
      naturalHeight: 8,
    } as HTMLImageElement;
    ...
    expect((gl as any).getAttribLocation).toHaveBeenCalledWith(
      program,
      "a_texCoord",
    );
    expect((gl as any).getUniformLocation).toHaveBeenCalledWith(
      program,
      "u_image",
    );
  ...
  it("updateVertexData uploads texcoords in addition to positions", () => {
    ...
    expect((gl as any).bindBuffer).toHaveBeenCalledWith(
      (gl as any).ARRAY_BUFFER,
      (img as any).texcoordBuffer,
    );
    expect((gl as any).bufferData).toHaveBeenCalledWith(
      (gl as any).ARRAY_BUFFER,
      (img as any).texCoordArray,
  ...
  it("determineIfLowRes returns true when area ratio below threshold", () => {
    ...
    (img as any).getBoundingBox = vi
      .fn()
      .mockReturnValue({ getArea: () => 100 } as any);
    expect(img.determineIfLowRes(camera, 1, 0.1)).toBe(true);
    // Above threshold
    (img as any).getBoundingBox = vi
      .fn()
      .mockReturnValue({ getArea: () => 2000 } as any);
  ...
  it("render initializes when not initialized and image ready", () => {
    ...
    expect(
      (img as unknown as ImageTestHook).updateVertexData,
    ).toHaveBeenCalledWith(gl);
  });
...
describe("getter and setter", () => {
  ...
  it("tests src setter", () => {
    ...
    expect(
      (img as unknown as ImageTestHook).updateImageTexture,
    ).toHaveBeenCalledWith("test_src");
...
describe("draw", () => {
  it("binds buffers, sets attributes and draws triangles", () => {
    ...
    expect(gl.bindBuffer).toHaveBeenNthCalledWith(
      1,
      gl.ARRAY_BUFFER,
      (img as unknown as ImageTestHook).positionBuffer,
    );
    expect(gl.vertexAttribPointer).toHaveBeenCalledWith(
      (img as unknown as ImageTestHook).attributeLocation,
      2,
      gl.FLOAT,
      false,
      0,
    ...
    expect(gl.bindBuffer).toHaveBeenNthCalledWith(
      2,
      gl.ARRAY_BUFFER,
      (img as unknown as ImageTestHook).texcoordBuffer,
    );
    expect(gl.vertexAttribPointer).toHaveBeenCalledWith(
      (img as unknown as ImageTestHook).texcoordLocation,
      2,
      gl.FLOAT,
      false,
      0,
    ...
    expect(gl.bindTexture).toHaveBeenCalledWith(
      gl.TEXTURE_2D,
      (img as unknown as ImageTestHook).texture,
    );

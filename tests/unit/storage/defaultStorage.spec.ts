...
vi.mock("../../../src/storage/dexie-primary-key.js", () => ({
  default: () => {},
}));
...
vi.mock("dexie", () => {
  ...
  class MockTable {
    ...
    where(field: string) {
      return {
        equals: (val: any) => ({
          first: async () =>
            Array.from(this.map.values()).find((e: any) => e[field] === val) ||
            null,
        }),
      };
    }
  ...
  MockDexie.prototype.version = function () {
    return { stores: (_schema: any) => {} };
  };
  MockDexie.prototype.open = async function () {
    /* noop */
  };
  MockDexie.prototype.transaction = async function (
    _mode: string,
    _table: any,
    fn: Function,
  ) {
  ...
  MockDexie.UUIDPrimaryKey = function (_db: any) {
    /* noop */
  };
  MockDexie.addons = [] as any[];
  MockDexie.override = (orig: any, wrap: (orig: any) => any) => wrap(orig);
  MockDexie.setByKeyPath = (obj: any, keyPath: string, value: any) => {
    obj[keyPath] = value;
  };
...
describe("DefaultIndexedDbStorage", () => {
  ...
  it("write stores an ImageFileMetadata and read retrieves it", async () => {
    ...
    (meta as any)._id = 7;
    const createSpy = vi
      .spyOn(ImageFileMetadata, "create")
  ...
  it("write respects max image entries limit and throws DatabaseLimitError", async () => {
    ...
    await expect(store.write(extraUrl)).rejects.toMatchObject({
      name: "DatabaseLimitError",
    });
    createSpy.mockRestore();
  });
});

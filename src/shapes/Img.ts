...
export class Img extends Rect {
  ...
  private async ensureLowResUploaded(gl: WebGLRenderingContext) {
    ...
    try {
      ...
      const scale = Math.min(
        1,
        targetMaxSide / Math.max(1, Math.max(naturalW, naturalH)),
      );
  ...
  private setLowResTextureFromBitmap(
    gl: WebGLRenderingContext,
    bitmap: ImageBitmap,
  ) {
    if (this.lowResTexture) {
      try {
        gl.deleteTexture(this.lowResTexture);
      } catch (e) {}
  ...
  protected draw(gl: WebGLRenderingContext) {
    ...
    try {
      const tex =
        this.useLowRes && this.lowResTexture
          ? this.lowResTexture
          : this.texture;

    ...
    switch (node.type) {
      ...
      case "Img":
        ...
        try {
          ...
          const framedPlaceholder = await framePlaceholder(
            src,
            width * node.transform.sx,
            height * node.transform.sy,
          );
          ...
          (instance as Img).fileId =
            (node as SerializedImg).fileId ?? (await hashStringToId(src));
          instance.setScale(node.transform.sx, node.transform.sy);
          canvas.appendChild(instance);
...
async function framePlaceholder(
  ...
  try {
    ...
    const targetW =
      tw && tw > 0 ? tw : (img?.naturalWidth ?? PLACEHOLDER_IMAGE_SIZE);
    const targetH =
      th && th > 0 ? th : (img?.naturalHeight ?? PLACEHOLDER_IMAGE_SIZE);
    ...
    if (img) {
      ...
      if (
        PLACEHOLDER_IMAGE_SIZE > ratioedHeight ||
